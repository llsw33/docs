...
- BR-004：本地运行使用内存缓存，不依赖外部缓存服务
- BR-005：卡片移动需携带来源列与目标列
- BR-006：同一用户在同一项目中的角色唯一

### 2.5 本地运行要求
- LR-001：数据库连接信息通过环境变量或配置文件提供
- LR-002：前端访问后端 API 需启用 CORS（若不同域）
- LR-003：WebSocket 需允许本地 origin（localhost）
- LR-004：后端启动前必须完成数据库初始化（DDL 或迁移）
...
- NFR-010：API 返回错误结构需统一（code/message/data）
